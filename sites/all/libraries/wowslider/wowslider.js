...
jQuery.fn.wowSlider = function (C) {
  ...
  if (C.preventCopy && !/iPhone/.test(navigator.platform)) {
    L = J('<div><a href="#" style="display:none;position:absolute;left:0;top:0;width:100%;height:100%"></a></div>').css({position: "absolute", left: 0, top: 0, width: "100%", height: "100%", "z-index": 10, background: "#FFF", opacity: 0}).appendTo(k).find("A").get(0)
  }
  var g = [];
  i.each(function (c) {
    var ai = J(">img:first,>a:first,>div:first", this).get(0);
    var aj = J("<div></div>");
    for (var f = 0; f < this.children[0].childNodes.length;) {
      if (this.children[0].childNodes[f].nodeType == 3) {
        aj.append(this.children[0].childNodes[f])
      } else {
        f++
      }
    }
  ...
  function q(f) {
    ...
    if (L) {
      ...
      if (c) {
        ...
        var rel = J(L).closest('div[id^="wowslider-container"]').find('.ws_images ul li a:first').attr('rel');
        L.setAttribute("rel", rel);
        if (Shadowbox != undefined && Shadowbox.cache[L.shadowboxCacheKey] != undefined) {
          Shadowbox.removeCache(Shadowbox.cache[L.shadowboxCacheKey].link)
          Shadowbox.setup();
        }
...
function wowsliderPreloader(m, d) {
  ...
  for (var o = 0; o < j.length; o++) {
    if (j[o]) {
      if (o >= g) {
        var c = f("<li>" + (j[o].href ? '<a href="' + j[o].href + '"' + (j[o].target ? ' target="' + j[o].target + '"' : "") + ">" : "") + '<img src="' + a + '"/>' + (j[o].href ? "ssssssssssss</a>" : "") + "</li>").appendTo(r);
